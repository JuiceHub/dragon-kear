...
def load_data(args, devices, kg):
    ...
    if args.local_rank != -1:
        _seed = args.seed + (
                2 ** args.local_rank - 1)  # use different seed for different gpu process so that they have different training examples
...
def calc_eval_accuracy(args, eval_set, model, loss_type, loss_func, debug, save_test_preds, preds_path):
    ...
    total_loss_avg, end_loss_avg, mlm_loss_avg, link_loss_avg, pos_link_loss_avg, neg_link_loss_avg, n_corrects_avg = \
        [item / n_samples_acm for item in (
            total_loss_acm, end_loss_acm, mlm_loss_acm, link_loss_acm, pos_link_loss_acm, neg_link_loss_acm,
            n_corrects_acm)]
...
def train(args, resume, has_test_split, devices, kg):
    ...
    # Separate the parameters into loaded and not loaded
    loaded_params, not_loaded_params, params_to_freeze, small_lr_params, large_lr_params = sep_params(model,
                                                                                                      loaded_roberta_keys)
    ...
    grouped_parameters = [
        ...
        {'params': [p for n, p in small_lr_params.items() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
         'lr': args.encoder_lr},
        ...
        {'params': [p for n, p in large_lr_params.items() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
         'lr': args.decoder_lr},
    ]
    ...
    if args.lr_schedule == 'fixed':
        ...
        try:
            ...
            scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=args.warmup_steps,
                                                        num_training_steps=max_steps, last_epoch=last_epoch)
    if resume:
    ...
    for epoch_id in trange(0, args.n_epochs, desc="Epoch", disable=args.local_rank not in [-1,
                                                                                           0]):  # trange(last_epoch + 1, args.n_epochs, desc="Epoch"):
        ...
        for qids, labels, *input_data in tqdm(dataset.train(steps=args.redef_epoch_steps, local_rank=args.local_rank),
                                              desc="Batch", disable=args.local_rank not in [-1, 0]):  # train_dataloader
            # labels: [bs]
        ...
        if args.local_rank in [-1, 0]:
            ...
            dev_total_loss, dev_end_loss, dev_mlm_loss, dev_link_loss, dev_pos_link_loss, dev_neg_link_loss, dev_acc = calc_eval_accuracy(
                args, dev_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path)
            print('dev_acc', dev_acc)
            if args.end_task and (args.mlm_task or args.link_task):
                dev_dataloader.set_eval_end_task_mode(True)
                _, dev_end_loss, _, _, _, _, dev_acc = calc_eval_accuracy(args, dev_dataloader, model, args.loss,
            ...
            if has_test_split:
                ...
                test_total_loss, test_end_loss, test_mlm_loss, test_link_loss, test_pos_link_loss, test_neg_link_loss, test_acc = calc_eval_accuracy(
                    args, test_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path)
                print('test_acc', test_acc)
                if args.end_task and (args.mlm_task or args.link_task):
                    test_dataloader.set_eval_end_task_mode(True)
                    _, test_end_loss, _, _, _, _, test_acc = calc_eval_accuracy(args, test_dataloader, model, args.loss,
                                                                                loss_func, args.debug, not args.debug,
                                                                                preds_path)
            ...
            if not args.debug:
                with open(log_path, 'a') as fout:
                    fout.write('{:3},{:5},{:7.4f},{:7.4f},{:7.4f},{:7.4f},{:3}\n'.format(epoch_id, global_step, dev_acc,
                                                                                         test_acc, best_dev_acc,
                                                                                         final_test_acc,
                                                                                         best_dev_epoch))

            wandb.log({"dev_acc": dev_acc, "dev_loss": dev_total_loss, "dev_end_loss": dev_end_loss,
                       "dev_mlm_loss": dev_mlm_loss, "dev_link_loss": dev_link_loss,
                       "dev_pos_link_loss": dev_pos_link_loss, "dev_neg_link_loss": dev_neg_link_loss,
                       "best_dev_acc": best_dev_acc, "best_dev_epoch": best_dev_epoch}, step=global_step)
            if has_test_split:
                wandb.log({"test_acc": test_acc, "test_loss": test_total_loss, "test_link_loss": test_link_loss,
                           "test_pos_link_loss": test_pos_link_loss, "test_neg_link_loss": test_neg_link_loss,
                           "test_end_loss": test_end_loss, "test_mlm_loss": test_mlm_loss,
                           "final_test_acc": final_test_acc}, step=global_step)
                if args.use_codalab:
                    with open("stats.json", 'w') as fout:
                        json.dump({'epoch': epoch_id, 'step': global_step, 'dev_acc': dev_acc, 'test_acc': test_acc},
                                  fout, indent=2)
            ...
            if (args.save_model == 2) or ((args.save_model == 1) and (best_dev_epoch == epoch_id)):
                ...
                try:
                    ...
                    pass
                checkpoint = {"model": model_state_dict, "optimizer": optimizer.state_dict(),
                              "scheduler": scheduler.state_dict(), "epoch": epoch_id, "global_step": global_step,
                              "best_dev_epoch": best_dev_epoch, "best_dev_acc": best_dev_acc,
                              "final_test_acc": final_test_acc, "config": args}
...
def evaluate(args, has_test_split, devices, kg):
    ...
    preds_path = os.path.join(args.save_dir, 'dev_e{}_preds.csv'.format(epoch_id))
    dev_total_loss, dev_end_loss, dev_mlm_loss, dev_link_loss, dev_pos_link_loss, dev_neg_link_loss, dev_acc = calc_eval_accuracy(
        args, dev_dataloader, model, args.loss, loss_func, debug, not debug, preds_path)
    if has_test_split:
        # Evaluation on the test set
        preds_path = os.path.join(args.save_dir, 'test_e{}_preds.csv'.format(epoch_id))
        test_total_loss, test_end_loss, test_mlm_loss, test_link_loss, test_pos_link_loss, test_neg_link_loss, test_acc = calc_eval_accuracy(
            args, test_dataloader, model, args.loss, loss_func, debug, not debug, preds_path)
    else:
        test_acc = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_model', default=2, type=float,
                        help="0: do not save model checkpoints. 1: save if best dev. 2: save always")
    ...
    parser.add_argument("--run_name", default="dragon", type=str,
                        help="The name of this experiment run.")
    ...
    parser.add_argument('--link_drop_max_count', type=int, default=100,
                        help='To specify #target positive triples for LinkPred')
    parser.add_argument('--link_drop_probability', type=float, default=0.2,
                        help='To specify #target positive triples for LinkPred')
    parser.add_argument('--link_drop_probability_in_which_keep', type=float, default=0.2,
                        help='Within target positive triples, how much to keep in the input graph?')
    ...
    parser.add_argument('--info_exchange', default=True, choices=[True, False, "every-other-layer"],
                        ...
                        help="Whether we have the MInt operator in every Fusion layer or every other Fusion layer or not at all.")
    ...
    parser.add_argument("--sep_ie_layers", default=False, type=utils.bool_flag,
                        help="Whether to share parameters across the MInt ops across differernt Fusion layers or not. Setting it to `False` means sharing.")
    ...
    parser.add_argument('--fp16', default=False, type=utils.bool_flag,
                        help='use fp16 training. this requires torch>=1.6.0')
    parser.add_argument('--upcast', default=False, type=utils.bool_flag,
                        help='Upcast attention computation during fp16 training')
