...
class MultiGPUSparseAdjDataBatchGenerator(object):
    ...
    def __init__(self, args, device0, device1, batch_size, indexes, qids, labels,
        ...
        self.span_len_dist = [self.geo_p * (1 - self.geo_p) ** (i - self.span_len_lower) for i in
                              range(self.span_len_lower, self.span_len_upper + 1)]
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            assert len(
                batch_tensors1) == 5  # tensors1: concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask
            ...
            yield tuple([batch_qids, batch_labels, batch_lm_inputs, batch_lm_labels, *batch_tensors0, *batch_lists0,
                         *batch_tensors1, *batch_lists1, edge_index, edge_type, pos_triples, neg_nodes])
    ...
    def process_lm_data(self, batch_tensors0):
        ...
        batch_lm_inputs, batch_lm_labels = self.mask_tokens(inputs=_inputs, mask_labels=_mask_labels,
                                                            special_tokens_mask=special_tokens_mask.view(-1, _seqlen))
    ...
    def process_graph_data(self, edge_index, edge_type, node_type_ids):
        ...
        input_edge_index = list(
            map(list, zip(*(iter(input_edge_index),) * nc)))  # nested list of shape (n_samples, num_choice)
...
class DRAGON_DataLoader(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
        ...
        self.train_qids, self.train_labels, self.train_encoder_data, train_concepts_by_sents_list = self.load_input_tensors(
            train_statement_path, max_seq_length, mode='train')
    ...
    def train(self, steps=-1, local_rank=-1):
        ...
        print('local_rank', local_rank, 'len(train_indexes)', len(train_indexes), 'train_indexes[:10]',
              train_indexes[:10].tolist())
        print('local_rank', local_rank, 'len(train_indexes)', len(train_indexes), 'train_indexes[:10]',
              train_indexes[:10].tolist(), file=sys.stderr)
        return MultiGPUSparseAdjDataBatchGenerator(self.args, self.device0, self.device1, self.batch_size,
                                                   train_indexes, self.train_qids, self.train_labels,
                                                   tensors0=self.train_encoder_data, tensors1=self.train_decoder_data,
                                                   adj_data=self.train_adj_data, tokenizer=self.tokenizer)

    def train_eval(self):
        return MultiGPUSparseAdjDataBatchGenerator(self.args, self.device0, self.device1, self.eval_batch_size,
    ...
    def dev(self):
        ...
        return MultiGPUSparseAdjDataBatchGenerator(self.args, self.device0, self.device1, self.eval_batch_size,
                                                   dev_indexes, self.dev_qids, self.dev_labels,
                                                   tensors0=self.dev_encoder_data, tensors1=self.dev_decoder_data,
                                                   adj_data=self.dev_adj_data, tokenizer=self.tokenizer)
    ...
    def test(self):
        ...
        if self.is_inhouse:
            return MultiGPUSparseAdjDataBatchGenerator(self.args, self.device0, self.device1, self.eval_batch_size,
                                                       test_indexes, self.train_qids, self.train_labels,
                                                       tensors0=self.train_encoder_data,
                                                       tensors1=self.train_decoder_data, adj_data=self.train_adj_data,
                                                       tokenizer=self.tokenizer)
        else:
            return MultiGPUSparseAdjDataBatchGenerator(self.args, self.device0, self.device1, self.eval_batch_size,
                                                       test_indexes, self.test_qids, self.test_labels,
                                                       tensors0=self.test_encoder_data, tensors1=self.test_decoder_data,
                                                       adj_data=self.test_adj_data, tokenizer=self.tokenizer)
    ...
    def load_input_tensors(self, input_jsonl_path, max_seq_length, mode='eval'):
        ...
        cache_path = input_jsonl_path + "-sl{}".format(max_seq_length) + (
            ("-" + self.model_type) if self.model_type != "roberta" else "") + '.loaded_cache'
        use_cache = True
    ...
    def load_sparse_adj_data_with_contextnode(self, adj_pk_path, max_node_num, concepts_by_sents_list, mode='eval'):
        ...
        cache_path = adj_pk_path + "-nodenum{}".format(max_node_num) + (
            "-cntsall" if self.cxt_node_connects_all else "") + '.loaded_cache'
        ...
        if use_cache:
            ...
            edge_index = list(map(list, zip(*(iter(
                edge_index),) * self.num_choice)))  # list of size (n_questions, n_choices), where each entry is tensor[2, E] #this operation corresponds to .view(n_questions, n_choices)
            edge_type = list(map(list, zip(*(iter(
                edge_type),) * self.num_choice)))  # list of size (n_questions, n_choices), where each entry is tensor[E, ]

            adj_lengths_ori, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask = [
                x.view(-1, self.num_choice, *x.size()[1:]) for x in
                (adj_lengths_ori, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask)]
            ...
            if mode == 'train' and self.args.local_rank != -1:
                assert len(adj_lengths_ori) == len(concept_ids) == len(node_type_ids) == len(node_scores) == len(
                    adj_lengths) == len(edge_index) == len(edge_type) == len(
                    special_nodes_mask)  # they equal to n_questions * num_choice
                ...
                assert len(adj_lengths_ori) == len(concept_ids) == len(node_type_ids) == len(node_scores) == len(
                    adj_lengths) == len(edge_index) == len(edge_type) == len(special_nodes_mask)
            ...
            if mode == 'train' and self.args.local_rank != -1:
                ...
                if rem != 0:
                    adj_concept_pairs = adj_concept_pairs + adj_concept_pairs[
                                                            : (self.args.world_size - rem) * self.num_choice]
            ...
            idx = -1
            for _idx, (_data, cpts_by_sents) in tqdm(enumerate(zip(adj_concept_pairs, concepts_by_sents_list)),
                                                     total=n_questions * self.num_choice, desc='loading adj matrices'):
                ...
                num_concept = min(len(concepts) + n_special_nodes,
                                  actual_max_node_num)  # this is the final number of nodes including contextnode but excluding PAD
            ...
            if not self.debug and self.args.dump_graph_cache:
                ...
                with open(cache_path, 'wb') as f:
                    ...
                    for _i_, obj in enumerate(
                            tqdm([adj_lengths_ori, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index,
                                  edge_type, half_n_rel, special_nodes_mask])):
                        pickle.dump({f'obj{_i_}': obj}, f, protocol=4)
                print('saved cache.', file=sys.stderr)
...
class InputExample(object):
    ...
    def __init__(self, example_id, question, contexts, endings, qc_meaning, ac_meaning,
                 label=None):
        ...
        self.label = label
        self.qc_meaning = qc_meaning
        self.ac_meaning = ac_meaning
        ...
        for json_dic in tqdm(lines):
            ...
            examples.append(
                InputExample(
                    ...
                    question="",
                    qc_meaning=[json_dic['question']['qc_meaning']] * len(json_dic["question"]["choices"]),
                    endings=[ending["text"] for ending in json_dic["question"]["choices"]],
                    ac_meaning=[ending["ac_meaning"] for ending in json_dic["question"]["choices"]],
                    label=label
                ))
    return examples
...
def simple_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, debug=False):
    ...
    for ex_index, example in tqdm(enumerate(examples), total=len(examples), desc="Converting examples to features"):
        if debug and ex_index >= debug_sample_size:
            break
        choices_features = []
        for ending_idx, (context, ending, qc, ac) in enumerate(
                zip(example.contexts, example.endings, example.qc_meaning, example.ac_meaning)):
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, max_seq_length, debug, tokenizer, debug_sample_size):
    ...
    if num_workers <= 1:
        ...
        with Pool(num_workers) as p:
            ress = list(p.starmap(simple_convert_examples_to_features,
                                  zip(examples_list, label_list_list, max_seq_length_list, tokenizer_list)))
